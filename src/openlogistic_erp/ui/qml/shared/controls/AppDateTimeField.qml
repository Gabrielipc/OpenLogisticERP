...
Item {
    ...
    function parseTextValue() {
        const match = /^(\d{2})\/(\d{2})\/(\d{4})\s+(\d{2}):(\d{2})$/.exec((input.text || "").trim())
        if (match) {
            return {
                day: Number(match[1]),
    ...
    function syncPopupState() {
        ...
        hourBox.value = parts.hour
        minuteBox.value = parts.minute
    }

    function formattedDateTime() {
        return `${String(root.selectedDay).padStart(2, "0")}/${String(root.selectedMonth).padStart(2, "0")}/${root.selectedYear} ${String(hourBox.value).padStart(2, "0")}:${String(minuteBox.value).padStart(2, "0")}`
    }
            ...
            RowLayout {
                ...
                AppButton {
                    ...
                    onClicked: {
                        const now = new Date()
                        root.selectDate(now)
                        hourBox.value = now.getHours()
                        minuteBox.value = now.getMinutes()
                    }
                }
                ...
                AppButton {
                    ...
                    variant: "contrast"
                    onClicked: {
                        root.text = root.formattedDateTime()
                        root.textEdited(root.text)
                        picker.close()
                    }
                }
